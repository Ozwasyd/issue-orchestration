...
`node-index.json` 只能登记 replay-verifiable 的 ledger head 和 node projection digest。Root 通过 `multi-node-state.mjs` replay control ledger 和每个 indexed node ledger，生成 `aggregate-runtime-projection.json`；不得把未验证 JSON summary 当作运行态。进程内 verified replay/projection cache 只能按 immutable authority 与 ledger-head identity 复用已经 replay 验证的 control/node components。Node append 只失效该 node；control append 只失效 control 与 aggregate，并复用 registration 未变的 node components。任何 startup/trust/package/policy authority、state root、run、node index 或 ledger head 变化都必须生成不同 key；进程重启、显式 audit、corruption suspicion 和最终 release/quiescence 必须从磁盘完整 replay。缓存不得持久化、不得读取调用者 projection、不得使用路径/mtime/时间戳作为 authority。单节点损坏只 quarantine 该节点，除非 dependency 或 acceptance-group invariant 要求传播 blocker。Push、issue close、delivery completion 与 cleanup finalization 仍由 control ledger exactly-once 序列化。
