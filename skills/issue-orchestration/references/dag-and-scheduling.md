...
`node-index.json` 只能登记 replay-verifiable 的 ledger head 和 node projection digest。Root 通过 `multi-node-state.mjs` replay control ledger 和每个 indexed node ledger，生成 `aggregate-runtime-projection.json`；不得把未验证 JSON summary 当作运行态。进程内 verified replay/projection cache 只能按 immutable authority 与 ledger-head identity 复用已经 replay 验证的 control/node components。Node append 只失效该 node；control append 只失效 control 与 aggregate，并复用 registration 未变的 node components。任何 startup/trust/package/policy authority、state root、run、node index 或 ledger head 变化都必须生成不同 key；进程重启、显式 audit、corruption suspicion 和最终 release/quiescence 必须从磁盘完整 replay。缓存不得持久化、不得读取调用者 projection、不得使用路径/mtime/时间戳作为 authority。由该 verified projection 编译出的 action set 还可在进程内按 selector receipt、remote snapshot receipt、semantic graph、aggregate projection、route policy、runtime capability 与 lifecycle authority 的精确 digest 复用；命中后仍须运行完整 action-set validator，并返回与缓存本体隔离的副本。任何 canonical append、scope/base、policy/capability 或 authority 漂移都必须换键，调用者编辑或复制 cache identity 不能授权执行。单节点损坏只 quarantine 该节点，除非 dependency 或 acceptance-group invariant 要求传播 blocker。Push、issue close、delivery completion 与 cleanup finalization 仍由 control ledger exactly-once 序列化。
...
`lifecycle-production-dispatcher.mjs` 是 action set 到 production owner 的唯一可执行桥梁。dispatcher 固定执行：复核 startup/trust authority → 重放 control/node ledgers → live scope/base freshness → 编译并逐字验证 action set → 按冻结穷尽映射执行 production owner → 通过专用 recorder 写入 verified event → 立即重放、重编译和补位。 每个 actor action 在 `contextProvider.prepare` 后、actor adapter prepare 前还必须由 `actor-context-envelope.mjs` 从同一 verified action/projection 编译 stage-specific envelope；executor 在 spawn 前再次绑定 action digest、role、phase 与 node。progressive source catalog 只留在 runtime 内存，actor 只能经 exact reference resolver 读取单块内容，不能枚举或批量继承完整 context。 dispatcher 随后用 `actor-prompt-cache-identity.mjs` 将同一 envelope 编译为 stable prefix 与 volatile suffix；stable prefix 只绑定角色/阶段/接口和不可变 package/policy/instruction 摘要，当前 run/node/route/slice/candidate/checkpoint/evidence 全部留在 suffix。adapter 无论是否支持 provider prompt cache 都必须收到逐字相同的 complete prompt；cache metadata 仅进入 diagnostic telemetry。pre-dispatch wave 对每个唯一 repository 只产生一个机器观察，并让精确匹配的 action 共享该 epoch；post-admission 边界同样按就绪 dispatch 集合去重仓库。epoch 只绑定当前 control-ledger head、repository/base binding、startup authority 和 action/dispatch 集合，不使用 TTL，也不授予 delivery、cleanup、retry、terminal 或 mutation authority。actor wave 必须只读取一次 shared verified projection；支持 batch 的 provider 通过一个 `dispatch-context-batch-request.v1` 返回逐 action digest 的有序 preparation，旧 provider 的单 action `prepare` 则并发执行。batch request 不携带 unrelated node projection；每项只绑定当前 action、node 与 shared repository-base epoch。dispatcher 在任何 actor spawn 前先验证 accepted subset 的 node-scoped attempt 以及 wave-unique slot/runtime/lease/resource identity，再一次性落盘 dispatch batch；prepare failure 只剔除自己的 action。actor attempt 必须在等待结果前落盘 slot/lease/resource/runtime binding；active dispatch 抑制重复派发。actor completion 进入的 ready queue 只存在于当前进程；至少一个 completion 可用后必须同步 drain 其他已就绪 completion，按 `dispatchId` 排序、共享一次 post-wave repository observation，并通过 canonical batch recorder 逐项复核和追加。未完成 actor 不得被等待；malformed/stale 项不得阻塞无关合法项，也不得写 node event。被拒绝的 promise 只有携带严格 `actor-stage-failure.v1`、匹配穷举 failure map、绑定原 persisted dispatch，并让内部完整 stage result 通过原有 writer/verifier validator 时，才可在同一 drained batch 中写精确 node event 和 `outcome=failed` settlement；任意异常、prose 或 control-plane failure 都不能被包装为节点失败。进程重启不得恢复任何旧 ready queue 或异常对象，只能重建 ledger 中仍 active 的 dispatch。compiler 明确标记为 batch-compatible 的 deterministic machine actions 由同一 shared projection 和 action set 驱动，使用不超过 runtime-observed local capacity 的 bounded worker pool 执行，并在任何 append 前完成 exact-result、node、action 与 stage validator 校验；当前仅 `compile-acceptance-contract` 可进入。结果按 `actionDigest` 稳定排序，通过一次 machine batch recorder 写 node events，malformed result 不得抹掉合法 peer，也不得写入自己的 node；该 pool 不占 actor slot，不取得 lease，不调用 model/network/Git/remote。Root 不得读取 action set 后自行选择 handler 或直接调用 action-family executor。canonical `idle` 只授权 quiescence finalizer，直到 `run.terminalized` 成功追加并重放才表示整轮终止。
