...
2. **编译下一动作**：每轮只能调用公共 `compileLifecycleActionSet`，输入 current selector/remote receipts、`semantic-graph.v2`、从 control/node ledgers 重放得到的 verified aggregate projection、installed route policy 与 runtime capability binding。Root 逐项机械执行返回的 `lifecycle-action-set.v1`，记录 verified result，重放 projection，再次调用 compiler，直到它返回 canonical quiescent `idle`。Root 不得手选 stage、手写 action、复制 projection JSON 或用 prose summary 替代 verified input。
3. **推进 test-contract 冷启动**：读取 [`references/dispatch-and-runtime-probe.md`](references/dispatch-and-runtime-probe.md)，对新节点只调用公共 `advanceTestContractColdStart`。该 API 依次验证 selector/remote facts 与 accepted semantic proposal，确定性编译 requirement inventory、acceptance contract 和 `node-discovered-receipt.v1`，请求 fresh observe-only `test-owner:test-contract-planning` 一次性返回 planning receipt、dispatch investigation 与 slice proposal，再确定性校验 slice/work plan/prompt。Writer 路由必须按 pending canonical selection → exclusive lease/resource → runtime-bound final route → `stage.contract-frozen` → distinct fresh writer dispatch 推进。Root 不得手拼 receipt、plan、slice、route、prompt、资源 authority 或 frozen history，也不得把完整 issue body 传给 writer。
...
7. **重放并继续**：每个 action 的结果必须写入对应 control/node ledger，重新生成 node index 与 aggregate projection，再次调用 `compileLifecycleActionSet`。除 compiler 返回 `idle` 外，Root 不得因为一个 stage 完成、一个节点关闭或长任务仍运行而停止循环。
