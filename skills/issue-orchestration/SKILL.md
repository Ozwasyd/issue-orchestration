...
5. **由 dispatcher 编译 actor 输入**：从 verified projection/action、plan/slice 与适用指令确定性生成 actor context envelope；模型只判断当前责任并返回 envelope 声明的 typed output。Root 不得手填、扩展或降级该输入。
