...
2. **进入唯一生产 dispatcher**：Root 只把 canonical ledger handle、startup authority 和受约束 runtime context provider 交给 `runLifecycleProductionDispatcher`。remote scope refresh 可使用可选 `observeRemoteIssueDelta`：request 必须绑定 prior selector/snapshot、lifecycle authority 与 adapter-owned cursor/conditional identity；exact `unchanged` 逐字复用旧 selector receipt、不得调用 selector 或追加 scope event，`changed` 必须提供完整 current issue identity set、完整 changed/new facts 与精确 removals，再由既有 canonical selector 重算。缺 cursor、无 delta API 或显式 `unsupported` 走完整观察；stale/partial/contradictory/wrong-authority response fail closed，wall-clock、ETag 文本、prompt 摘要或本地 cache 都不是 unchanged authority。dispatcher 每轮自行复核 startup/trust/package/policy authority，重放 control/node ledgers，执行 live scope/base freshness，调用唯一 lifecycle compiler，逐字验证 action set，并通过冻结的穷尽映射交给对应 production owner。同一 dispatcher wave 内，所有绑定相同 repository/base/head/authority 的 action 必须共享一个 `repository-base-observation-epoch.v1`；跨仓观察可并发，但 control-ledger head、scope、base、repository identity 或 authority epoch 任一变化都会使旧 epoch 失效。actor wave 必须从同一 verified projection 和 action set 进行准备：provider 支持 `prepareBatch` 时只调用一次，否则对旧 `prepare` 并发调用；batch API 只接收当前 action 的受限 node projection 与精确 repository-base epoch binding。所有 accepted preparation 的 node-scoped attempt、slot、runtime、lease、resource 与 action identity 必须在任何 dispatch append 或 actor spawn 前完整验证，随后一次性落盘 dispatch batch 再并发执行；单项准备失败只能剔除该 action。任一结果到达后立即录入、释放精确槽位、重放并补位。Root 不得读取 action set 后自行 switch、调用 action-family executor、等待整批完成、复制 projection JSON，或用 prose summary 替代 verified input。
