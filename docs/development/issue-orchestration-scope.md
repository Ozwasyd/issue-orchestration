...
- first required action、read targets、允许写路径或 read-only output、forbidden paths；
...
上述八个 schema 与 compiler/progress API 是永久 runtime 合同。某次 repair issue 直接授权 Sol Ultra 完成代码、文档和交付，只是该批次的实现授权，不能写入永久 `stage-model-pool.v2`、routing 或 fallback policy。任何临时 bootstrap run、旧 receipt、first failure 与 breaker 只保留作冻结测试恢复、历史审计 fixture 和最终退役证据；它们不是永久 dispatcher、执行前置或兼容入口。
...
只有 `semanticGraphInputDigest` 发生未预期变化（dependency、owner、conflict、contract 或 acceptance-group 等 semantic fact）时，决策才是 `semantic-patch`，并且必须授权恰好一次 fresh、read-only DAG updater。普通 semantic update 只能返回 `issue-orchestration.semantic-graph-patch.v1`：patch 绑定 base `semanticGraphDigest`、允许的最小 operation、直接 evidence digests、可重算的 result digest，且不得嵌入 full graph；root 只能原样接受已封存 patch，不能自行 author/edit，应用必须经过 root-acceptance gate。`full-create` 仅用于 initial create；`full-recovery` 仅用于带 machine evidence 的 `graph-corruption-recovery` 或显式 `scope-replacement`。其它 full proposal、base drift、非法 operation、结果 digest 不匹配或 root-authored patch 必须 fail closed。
...
每次阶段派发都必须先生成不可变的 `issue-orchestration.dispatch-request.v2`。Writer request 在原有 `stage-model-pool.v2` stage/permission identity 外，还必须绑定 `execution-capability-routing.v1` policy digest、verified plan/slice、`execution-shape-classification.v1`、`stage-capability-requirement.v1`、唯一 `profile-capability-matrix.v1` evidence digest、`execution-route-decision.v1`、compiled prompt、requested/effective runtime identity 和 group/member/lease continuity。完整 issue、Root 手选 profile、成本/余额/token/telemetry/human preference、按失败次数升档或 silent fallback 都不能生成合法 request。`requestDigest` 覆盖去掉自身后的完整 request；完整 prompt、credential 和 secret environment 不进入 receipt；request/receipt 只写仓库外状态根。

Root 启动还必须先提交 `issue-orchestration.root-startup-request.v2`，并由真实 root rollout 和受信 machine capability observation 生成 `issue-orchestration.root-startup-receipt.v2`。root 的唯一合法 profile 是 `luna-low`（实际 `gpt-5.6-luna`、`low`、`normal`、`root-scheduler`）；缺少可观察的 thread/rollout、cwd、model、effort、mode 或 capability 时只能是 `capability-unverified`，任何 profile/role/mode/cwd 不匹配均为 `rejected`，在 receipt verified 前不得启动 DAG dispatch。
...
永久 writer route 的唯一 correctness authority 是 `execution-capability-routing.v1`。它消费 #1874 verified slice 与机器 execution metrics，产生 shape、capability requirement 和 route decision；profile matrix 只能由 package 中唯一的可重算 machine runtime observation fixture 维护。Atomic/低深度 bounded 可选 Luna，evidenced runtime/lifecycle 可选 Terra，复杂可切片/多模块/深工具链选 Sol/high，只有机器证明不能继续安全切片的长链或唯一能力匹配才选 Sol/xhigh，Sol/max 仍只允许 frontier exception。`engineeringRiskClass=high-risk` 不是 Terra/max 开关。UI implementation 永久限 `sol-low`/`sol-medium`，超出能力必须重切片或停止进入 fresh read-only adjudication。
...
Documentation 仅在 behavior-green，且 UI work 已 ux-accepted 后启动；普通同步使用 `luna-high`，跨文档 authority 迁移可使用 `luna-max`，不得按余额、失败次数、reworkCount、telemetry 或人工偏好切换。FsusBlog UI/UX stage 必须从机器 load evidence 取得 `fsusblog-design-conformance` 与 `fsusui-design-conformance` 的 exact digest。`uiDecisionClass=system-design-dispute` 时必须先由 fresh read-only `ui-system-adjudicator` 使用 `sol-high`/`sol-xhigh` 输出 machine-readable owner、边界、design authority 和允许的 low/medium implementation class；裁决者没有实现权。behavior verification、UX acceptance 和 adjudication 都必须 fresh、read-only 且不继承 implementer 对话。group continuity 必须重新按 member 分类和 routing，不能继承上一 member 的 profile、权限或 verifier context。
...
dispatch receipt `issue-orchestration.dispatch-receipt.v2` 只有在 request digest、runtime metadata digest、thread/rollout identity、prompt/DAG/frontier/base/candidate/epoch、routing policy/input、effective model/effort/role/mode/sandbox/fork/cwd/profile、Skills/capability 和 group/member/lease 全部一致时才是 `verified`。`implementation.started` 必须引用 verified v2 dispatch receipt；`implementation.candidate-green` 必须引用同一 request/base/candidate/attempt/epoch 的 `issue-orchestration.implementer-self-test-receipt.v2`，其 visible matrix、命令结果、failure history/fix cycle、frozen test tree、working-tree 和 modified paths 均由 deterministic machine verifier 封存。`independent-verification.passed` 只能引用 `issue-orchestration.behavior-receipt.v2`，且必须由 fresh、read-only 的 `test-owner` 产生；不得用 self-test receipt、implementer 自述或旧 verifier 代替。v1 dispatch/self-test/behavior receipts 仅可读取为 historical evidence，不能授权任何 v2 transition。
...
`candidate-green` 原子撤销 implementer writer，并只为同一 candidate 授予 independent verifier read-only lease。verification rejection 撤销 reader，并在同一 worktree/attempt 上恢复唯一 writer；双 writer、新 worktree 或丢失 retained service descendant/port owner 都 fail closed。
...
调度 Skill、运行脚本、策略、图 schema、writer plan/slice/prompt/checkpoint/failure 与 execution-routing schemas、模板和 agent 定义只有一个可编辑 authoring source：``。`manifest.json` 是该 package 的身份和完整性边界，绑定 `sourceCommit`、`sourceTreeDigest`、每个 artifact digest、`manifestDigest`、stage/model permission、execution-routing policy、唯一 capability matrix/evidence、semantic graph/runtime projection、writer compiler/progress API 及十二个永久 writer/routing schemas、projector、两个支持仓库和所需 capabilities。manifest 不能把自己列入 `artifactDigests`；任一绑定或 canonical SHA-256 不匹配都必须拒绝。

Package 对外只发布七个角色：`code-implementer`、`dag-creator-updater`、`documentation-writer`、`test-owner`、`ui-system-adjudicator`、`ui-ux-implementer` 和 `ux-acceptance-verifier`。Landing conflict resolution 只能复用 code/UI writer，不发布 `landing-owner`。角色与 profile 由单一 `stage-model-pool.v2` 和 deterministic routing compiler 选择：root scheduler 固定 `luna-low`（`gpt-5.6-luna`/`low`），documentation 只能使用 `luna-high` 或 `luna-max`，cleanup 的 green authority 是 machine resource verifier。旧的 `.agents/skills/issue-orchestration`、`.codex/agents` issue aliases、review/cleanup-verifier aliases、临时 bootstrap dispatcher 和 fixed-Sol/node-local profile authority 都不是可发现的兼容来源；产品 API、design、documentation 和 runtime authority 仍由各仓库 current 文档/AGENTS 在运行时提供，不能复制进 package。
